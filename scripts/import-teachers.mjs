...
const CODES = {
  ...
  // Ольг К. під цим кодом троє, і предмет їх не розводить: англійську
  // ведуть двоє, тож Козак і Кравчишин розходяться класами. Хто саме веде
  // кожен урок — з учительського розкладу, де кожна сторінка підписана
  // повним іменем. Українську веде третя, Курман, а не Козак.
  ОК: [
    { who: 'Кравчишин Ольга Петрівна', when: [{ s: ['ам'], in: ['5д', '7а', '7б'] }] },
    { who: 'Козак Ольга Ігорівна', when: [{ s: ['ам'], in: ['4б', '4в', '6а', '6б'] }] },
    { who: 'Курман Ольга Михайлівна', when: [{ s: ['ум'] }] },
  ],
  ОБ: [
    { who: 'Борковська Олена Вадимівна', when: [{ s: ['ам'] }] },
    { who: 'Боцюрко Оксана Мирославівна', when: [{ s: ['ум', 'ул'] }] },
  ],
  // Ганну Пірановську розклад не підписує взагалі: її 34 уроки стоять без
  // коду — найпевніше тому, що «ГП» уже зайняте двома іншими. Код їй
  // проставляє імпорт розкладу (див. `vchyteli.mjs`), а хто це — видно з
  // учительського розкладу, де кожна сторінка підписана повним іменем.
  ГП: [
    { who: 'Пташник Галина Василівна', when: [{ s: ['ам'] }] },
    { who: 'Перцович Галина Ігорівна', when: [{ s: ['ум', 'ул'] }] },
    { who: 'Пірановська Ганна Василівна', when: [{ s: ['М', 'і'] }] },
  ],
 ...
 *
 * ОГ — фізкультура в 5-Б, 5-В і 9-Б, 9 уроків. Це Орест Горбатий: так
 *      підписана його сторінка в учительському розкладі, і класи там ті
 *      самі. У журналі ліцею його немає, а без запису звідти немає й `id`,
 *      тож у довіднику код поки лишається кодом. Здогад про Ольгу Галюк був
 *      неправильний; Олена Горбата — теж не він, вона веде англійську в 1–3.
 */
const UNDECODED = {
...
function render(people) {
  const undecoded = Object.entries(UNDECODED)
    .map(([code, u]) => {
      const parts = [`subject: ${q(u.subject)}`]
      if (u.last) parts.push(`last: ${q(u.last)}`, `first: ${q(u.first)}`)
