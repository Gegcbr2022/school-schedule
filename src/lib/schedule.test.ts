...
describe('групи 10-Б', () => {
  ...
  /*
   * Пів клітинки без підпису групи — це урок раз на два тижні. Десять
   * таких стоять останніми в дні й приходять із класного PDF. Ще двадцять
   * додав учительський розклад: географія проти другої іноземної в 9-А і
   * 9-Б та інформатика, на яку групи 8-х і 9-х ходять по черзі. Поки їх не
   * розпізнавали, учні бачили їх щотижня. Тест не дасть цьому повторитись.
   */
  it('уроки «через тиждень» розпізнані в усіх класах, не лише в 10-Б', () => {
    ...
    // Географія через тиждень із другою іноземною — з учительського розкладу.
    expect(fortnightly).toContain('9а 2 2 г 2')
    expect(fortnightly).toContain('9а 2 2 нм 1')
    expect(fortnightly).toContain('9б 4 3 г 2')
    // Інформатика: перша група — на першому тижні, друга — на другому.
...
describe('п’ятниця 10-Б', () => {
  it('останній урок — по групах', () => {
    ...
    // Кабінет — за папером зі стенду: 14 країнознавству, 34 технологіям.
    expect(at10b(G1, FRI, 7)?.items[0].room).toBe('14')
    expect(at10b(G2, FRI, 7)?.items[0].room).toBe('34')
  })
})
